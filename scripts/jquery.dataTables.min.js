...
nPrevious.className=oSettings.oClasses.sPagePrevDisabled;nNext.className=oSettings.oClasses.sPageNextDisabled;nPrevious.title=oSettings.oLanguage.oPaginate.sPrevious;nNext.title=oSettings.oLanguage.oPaginate.sNext;nPaging.appendChild(nPrevious);nPaging.appendChild(nNext);$(nPrevious).click(function(){oSettings.oApi._fnPageChange(oSettings,"previous");fnCallbackDraw(oSettings);});$(nNext).click(function(){oSettings.oApi._fnPageChange(oSettings,"next");fnCallbackDraw(oSettings);});$(nPrevious).bind('selectstart',function(){return false;});$(nNext).bind('selectstart',function(){return false;});if(oSettings.sTableId!==''&&typeof oSettings.aanFeatures.p=="undefined")
...
{var oSettings=_fnSettingsFromNode(this[_oExt.iApiIndex]);_fnPageChange(oSettings,sAction);if(typeof bRedraw=='undefined'||bRedraw)
{_fnReDraw(oSettings);}};function _fnExternApiFunc(sFunc)
...
var iThisIndex=oSettings.aoData.length;oSettings.aoData.push({"nTr":document.createElement('tr'),"_iId":oSettings.iNextId++,"_aData":aData.slice(),"_anHidden":[],"_sRowStripe":''});var nTd;for(var i=0;i<aData.length;i++)
...
{alert("DataTables warning: Unexpected number of TD elements. Expected "+nTds.length+" and got "+(nTrs.length*oSettings.aoColumns.length)+". DataTables does not support "+"rowspan / colspan in the table body, and there must be one cell for each row/column "+"combination.");}
...
{oSettings._iDisplayStart=oSettings.iInitDisplayStart;oSettings.iInitDisplayStart=-1;_fnCalculateEnd(oSettings);}
...
var iClass=1;var nTrs=_fnGetTrNodes(oSettings);for(i=0;i<aaSort.length;i++)
