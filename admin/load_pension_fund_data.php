...
<head>
<title>Загрузить данные по стоимости КУПА</title>
...
<div class="title"><a class="more" href="index.php">Панель администратора</a>Загрузить данные по стоимости КУПА</div>
...
while (false != ($line = fgets($fp,4096)))
{
           ...
           {
             //update table
           	 $query='insert into ism_pension_fund_value(fund_id,check_date,value,asset_value) values('.$data[$i][1].',\''.substr($data[$i][2],6,4).'-'.substr($data[$i][2],3,2).'-'.substr($data[$i][2],0,2).'\','.$data[$i][3].','.$data[$i][4].')';
